<?php
//clase alumno con metodos

require_once('conexion.php');
  
	class AlumnoDB
	{
	function guardar_datos($nombre,$apellido,$direccion,$cedula,$telefono,$email,$eCivil,$fechanacimiento,$password)
 	{
	$config=new conexion();
	$link=mysqli_connect($config->get_dbhost(),$config->get_dbuser(),$config->get_dbpass(),$config->get_dbname());	
		
	$exist = mysqli_num_rows( mysqli_query($link,"SELECT * FROM alumnos WHERE alu_cedula = '$cedula'"));
	
	if($exist==0){
		$sql= "insert into alumnos (alu_Nombre,alu_Apellido,alu_direccion,alu_telefono,alu_email,alu_ecivil,alu_fnacimiento,alu_pass,alu_cedula)values('$nombre','$apellido','$direccion','$telefono','$email','$eCivil','$fechanacimiento','$password','$cedula')";
	    $result = mysqli_query($link,"SET NAMES 'utf8'");	
	    $result=mysqli_query($link, $sql) or die (mysqli_error($link));
	    
	    
		
		echo("Lista de Alumno <br>");
		$resultados = mysqli_query($link,"SELECT * FROM alumnos");
		while($consulta = mysqli_fetch_array($resultados))
		{
		echo ("Nombre:".$consulta['alu_Nombre']."<br>"); 
		echo ("Apellido:".$consulta['alu_Apellido']."<br>"); 
		echo ("Direccion:".$consulta['alu_direccion']."<br>"); 
		echo ("Cedula:".$consulta['alu_cedula']."<br>"); 
		echo ("Telefono:".$consulta['alu_telefono']."<br>"); 
		echo ("Email:".$consulta['alu_email']."<br>"); 
		echo ("Estado Civil:".$consulta['alu_ecivil']."<br>"); 
		echo ("Fecha de Nacimiento:".$consulta['alu_fnacimiento']."<br>"); 
		echo ("Password:".$consulta['alu_pass']."<br>"); 
		echo ("<br>");
		}
		return $result;
		mysqli_close($link); 
	}
	else{
		echo("El alumno ya esta ingresado");
	}
   
	}
		
}
?>